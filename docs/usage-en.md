...
```sh
# Pin a version
bash /tmp/ferri-install.sh 0.1.0

# Choose directories (absolute paths required)
FERRI_INSTALL_DIR="$HOME/tools/bin" FERRI_DATA_DIR="$HOME/tools/share" bash /tmp/ferri-install.sh
```

The default executable is `~/.local/bin/ferri`, with completion scripts under `~/.local/share`. Use `FERRI_VERSION` to select a version and `FERRI_REPOSITORY` to select a GitHub repository. On other Unix platforms, build from source, run `bash install.sh --local`, and provide compatible device tools yourself.

On Windows, choose a version and directory:

```powershell
powershell -ExecutionPolicy Bypass -File "$env:TEMP\ferri-install.ps1" -Version 0.1.0 -InstallDir "$env:LOCALAPPDATA\Ferri"
```
...
```sh
source "$HOME/.local/share/bash-completion/completions/ferri"
```
...
```fish
source ~/.local/share/fish/vendor_completions.d/ferri.fish
```
...
```powershell
. "$env:LOCALAPPDATA\Ferri\ferri-completion.ps1"
```

For a Homebrew installation, use:

```powershell
. "$(brew --prefix)/share/pwsh/completions/ferri.ps1"
```

Run `ferri __completion bash` to print the script, or replace `bash` with `zsh`, `fish`, or `powershell`. Use the space-separated form `--target PATH` for path completion. The command also accepts `--target=PATH` and `--device=ID`.

## Environment settings

| Variable | Purpose |
| --- | --- |
| `FERRI_HOME` | Tool cache directory; defaults to `~/.ferri` |
| `FERRI_ADB` | Path to the adb executable |
| `FERRI_IOS` | Path to the go-ios executable |
| `FERRI_JAVA` | Path to a Java 17+ executable |
| `FERRI_BUNDLETOOL` | Path to the bundletool JAR |

Ferri also reuses PATH, ANDROID_HOME, ANDROID_SDK_ROOT, JAVA_HOME, and Android, Java, and bundletool caches from the original script under `~/.app-installer`. Prefer absolute paths when specifying tool files.

If tools are missing, run `ferri --target PATH` interactively and approve the download. Prepare tools in advance for non-interactive sessions. `--device` skips device selection only; it does not authorize dependency downloads.

## Troubleshooting

| Situation | What to check |
| --- | --- |
| `--list` reports missing tools | Run `ferri --target PATH` to review and approve the required tools, or point to existing tools through environment variables |
...
| An update fails with a signing mismatch | Prefer a package signed with the same key to preserve data. After failure, Ferri checks for a matching app and offers uninstall/reinstall with a data-loss warning; the default is no. |
...
An IPA needs a valid signature and provisioning profile for the target device. System drivers and services cannot be replaced by user-space tools in Ferri's cache. See [Android bundletool](https://developer.android.com/tools/bundletool) and [go-ios](https://github.com/danielpaulus/go-ios).
...
On Windows, run `go build -o bin/ferri.exe .`, then `./tests/windows_test.ps1`. GitHub Actions defines tests for all three platforms and Homebrew installation checks. Tests use simulated devices and do not install apps on real devices.

The original script is retained at [legacy/install-app.sh](../legacy/install-app.sh). The root `install.sh` installs Ferri itself. Preserve the existing code style and do not run formatters. See [RELEASING.md](../RELEASING.md) for the release process in Chinese.

## URL installation

```sh
ferri --url "https://example.com/app.apk"
ferri --url "https://www.pgyer.com/3ceb0fd1d20e97fa0310d802323c07c8" --android
ferri --url "https://www.pgyer.com/clobotics-rea-test" --ios --device UDID
ferri --url "https://play.google.com/store/apps/details?id=com.example.app" --device SERIAL
```

- **Direct downloads:** HTTP(S), relative redirects, session cookies and extensionless URLs are supported. ZIP contents determine APK / APKS / AAB / IPA type; URL extensions and server filenames are not trusted. Ferri tries updating first. Only after failure and detection of the same applicationId / bundleId does it ask for permission to uninstall/reinstall, warning that local data will be deleted.
...
- **App Store:** app links are not IPA download URLs. Ferri recognizes the link and returns an explanation. Install through App Store on the device, or provide a signed IPA suitable for the device.
...
Downloads require no extra tools. Limits: 15 minutes per request, 12 page/redirect hops, 2 MiB per page and 8 GiB per package. Packages are stored in the system temporary directory and removed after normal completion, download failure or installation failure. Forced termination or shutdown can leave `ferri-download-*` directories for manual cleanup. Download logs omit URL query parameters. `--url=URL` is supported; quote URLs. `--android` / `--ios` require `--url` and are mutually exclusive.
...
Before installation, Ferri restarts the selected adb server once (`kill-server` / `start-server`) when discovery fails, returns an empty list, or reports the target as offline. Each server command has a 10-second timeout; device re-enumeration is then allowed up to 5 seconds. An explicit device ID is never replaced by another device. Without an explicit ID, discovery does not restart a server that still lists usable devices or devices awaiting authorization.

Ferri prints a notice before restarting; other debugging sessions on that adb server may briefly disconnect. If recovery fails, check the USB connection, USB debugging and device authorization. Unauthorized devices still require approval on the device. Listing, completion and iOS installation do not trigger recovery. Failed uninstall/install commands are not automatically retried.
