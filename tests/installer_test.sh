...
export FERRI_INSTALL_DIR="$TEMP/bin space"
export FERRI_DATA_DIR="$TEMP/share space"
export FERRI_TEST_FIXTURES="$TEMP/releases"
mkdir -p "$FERRI_TEST_FIXTURES" "$TEMP/tools"
case "$(uname -s)" in Darwin) platform=darwin ;; Linux) platform=linux ;; *) exit 0 ;; esac
case "$(uname -m)" in arm64|aarch64) arch=arm64 ;; *) arch=amd64 ;; esac
version="$(cat "$ROOT/VERSION.txt")"
expected_output="$("$ROOT/bin/ferri" --version)"
asset="ferri_${version}_${platform}_${arch}"
cp "$ROOT/bin/ferri" "$FERRI_TEST_FIXTURES/$asset"
printf '%s\n' "$version" > "$FERRI_TEST_FIXTURES/VERSION.txt"
if command -v shasum >/dev/null; then
    digest="$(shasum -a 256 "$ROOT/bin/ferri" | awk '{print $1}')"
else
    digest="$(sha256sum "$ROOT/bin/ferri" | awk '{print $1}')"
fi
printf '%s  %s\n' "$digest" "$asset" > "$FERRI_TEST_FIXTURES/SHA256SUMS"
